...
class OhemCELoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        # max_val5 = torch.max(labels)
        # min_val5 = torch.min(labels)

        loss = self.criteria(logits, labels).view(-1)       # 转为1维张量
...
if __name__ == '__main__':
    ...
    weighted_criteria1 = WeightedOhemCELoss(thresh=0.7, n_min=16 * 20 * 20 // 16,num_classes=2).cuda()
    weighted_criteria2 = WeightedOhemCELoss(thresh=0.7, n_min=16 * 20 * 20 // 16,num_classes=2).cuda()
    ...
    loss1 = weighted_criteria1(logits1, lbs)
    loss2 = weighted_criteria2(logits2, lbs)
